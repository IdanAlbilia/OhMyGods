...
func _setup_anim_sprite():
	...
	if is_soldier:
		# Rebuild from the imported texture image so filtering cannot bleed across frame boundaries.
		var soldier_sheet := load("res://assets/characters/sheets/soldier_sheet.png") as Texture2D
		if soldier_sheet == null:
			return
		var img := soldier_sheet.get_image()
		if img == null:
			return
		...
		texture = load("res://assets/characters/sheets/believer_sheet_v2.png")
		fw = FRAME_W
		fh = FRAME_H
