...
func _spawn_trees() -> void:
	var tex15 : Texture2D = load("res://assets/environment/Comp15.png")
	var tex16 : Texture2D = load("res://assets/environment/Comp16.png")
...
func _spawn_village_houses() -> void:
	var tex : Texture2D = load("res://assets/buildings/Comp1.png")
	if tex == null:
		return
	var target_w := 92.0
...
func _spawn_well() -> void:
	var tex : Texture2D = load("res://assets/buildings/Comp10.png")
	if tex == null:
		return
	var s := Sprite2D.new()
	s.texture = tex
...
func _spawn_blockade_guards() -> void:
	for gx in [BLOCKADE_POS.x - 78.0, BLOCKADE_POS.x + 64.0]:
		var g : CharacterBody2D = preload("res://scenes/believer.tscn").instantiate()
		g.is_soldier = true
		add_child(g)
...
func _spawn_villagers() -> void:
	for i in range(TOTAL_VILLAGERS):
		var v : CharacterBody2D = preload("res://scenes/believer.tscn").instantiate()
		v.setup(VILLAGER_HOMES[i], i)
		add_child(v)
		villager_nodes.append(v)

func _spawn_preacher() -> void:
	preacher_node = preload("res://scenes/believer.tscn").instantiate()
...
func _spawn_soldier() -> void:
	soldier_node = preload("res://scenes/believer.tscn").instantiate()
